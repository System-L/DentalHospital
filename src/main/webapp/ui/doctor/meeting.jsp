...
<!-- 预约记录查看的模态框 -->
<!-- Modal -->
<div class="modal fade" id="doctorSeeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title">就诊</h4>
            </div>
            <div class="modal-body">
                <div class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="text" readonly="readonly" name="customer" class="form-control"
                                   id="customer_set_input"/>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="text" readonly="readonly" name="docotor" class="form-control"
                                   id="doctor_set_input" value="${doctorSession.name }"/>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">备注:</label>
                        <div class="col-sm-10">
                            <textarea name="remark" class="form-control" id="remark_set_input"
                                      style="height:420px;"></textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">其他:</label>
                        <button class="btn btn-primary col-sm-2 bill_btn" id="hospitalization_set_input">医疗项目</button>
                        <button class="btn btn-primary col-sm-2 bill_btn" id="medicine_set_input">药物</button>
                        <label class="col-sm-3 control-label">合计(元):</label>
                        <div class="col-sm-3">
                            <input type="text" readonly="readonly" name="cost" class="form-control" id="bill_set_input">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" data-dismiss="modal" id="see_save_btn">确定</button>
            </div>
        </div>
    </div>
</div>
<!-- 医疗项目查看的模态框 -->
<!-- Modal -->
<div class="modal fade" id="hospitalizationAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title">添加医疗项目</h4>
            </div>
            <div class="modal-body">
                <div class="form-horizontal">
                    <div class="form-group">
                        <div class="form-horizontal">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">医疗项目：</label>
                                <div class="col-sm-4">
                                    <select type="text" name="hospitalization_select" class="form-control" id="hospitalization_select_static"></select>
                                    <span class="help-block"></span>
                                </div>
                                <button class="btn btn-primary col-sm-1" id="hospitalization_add_btn">添加</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style="height:508px;">
                <div class="col-md-12">
                    <table class="table table-hover" id="hospitalization_table">
                        <thead>
                        <tr>
                            <th>#</th>
                            <th>项目名称</th>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" data-dismiss="modal" id="hospitalization_pay_btn">确定</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="medicineAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-horizontal">
                    <div class="form-group">
                        <div class="form-horizontal">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">药品项目：</label>
                                <div class="col-sm-4">
                                    <select type="text" name="hospitalization_select" class="form-control" id="medicine_select_static"></select>
                                    <span class="help-block"></span>
                                </div>
                                <button class="btn btn-primary col-sm-1" id="medicine_add_btn">添加</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary" data-dismiss="modal" id="medicine_pay_btn">确定</button>
            </div>
        </div>
    </div>
</div>
        ...
        <nav id="sidebar" role="navigation" data-step="2"
            ...
            <div class="sidebar-collapse menu-scroll">
                <ul id="side-menu" class="nav">
                    ...
                    <li><a href="goods.jsp"><i class="glyphicon glyphicon-plus">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 col-md-offset-4">
                        <div class="form-group">
                            ...
                            <div class="col-sm-1">
                                <button class="btn btn-primary" id="meet_search_modal_btn">查询</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function to_page(pn) {
        ...
        if (customerName != "") {
            var Url = "${APP_PATH }/meetsDoctor/search";
            var data = "pn=" + pn + "&customerName=" + customerName;
        } else {
            var Url = "${APP_PATH }/meetsDoctor";
            var data = "pn=" + pn;
        }
        $.ajax({
            url: Url,
        ...
        $.each(meets, function (index, item) {
            ...
            if (item.status == '0') {
                ...
                var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
                    .append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("叫号");
            } else if (item.status == '1') {
                var statusTd = $("<td></td>").append("待就医");
                var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_model_btn")
                    .append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("就医");
            } else if (item.status == '2') {
                var statusTd = $("<td></td>").append("已就医");
                var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
                    .append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("取药");
            } else {
    ...
    //根据姓名查询预约
    $("#meet_search_modal_btn").click(function () {
        to_page(1);
    });

    //今日预约
    $("#meet_today_modal_btn").click(function () {
        if (this.checked == true) {
            $.ajax({
                url: "${APP_PATH }/meetsDoctor/today",
                data: "pn=" + 1,
                type: "GET",
                success: function (result) {
                    //1、解析并显示就诊记录数据
    ...
    $(document).on("click", ".edit_btn", function () {
        ...
        $.ajax({
            url: "${APP_PATH }/meetsDoctor/edit/" + $(this).attr("edit-id"),
            type: "GET",
            success: function (result) {
                if (result.code == 100) {
                    alert("叫号成功");
    ...
    function getMeet(id) {
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#id_set_input").val(id);
                $("#customer_set_input").val(meetData.customerName);
                $("#date_set_input").val(new Date(parseInt(meetData.date)).toLocaleDateString().replace(/:\d{1,2}$/, ' '));
            }
        })
    }

    //添加医疗信息
    $(document).on("click", "#hospitalization_set_input", function () {
        //1.医疗信息点击 需弹出 模态框
        getHospitalization("#hospitalizationAddModal select");
        $("#hospitalizationAddModal").modal({
            backdrop: "static"
        });
    });

    //获取所有医疗项目
    function getHospitalization(ele){
        //清空之前下拉列表的值
        $(ele).empty();
        $.ajax({
            url:"${APP_PATH}/hospitalization",
            type:"GET",
            success:function(result){
                //显示部门信息在下拉列表
                $.each(result.extend.hospitalization,function(){
    ...
    //添加医疗项目
    $("#hospitalization_add_btn").click(function(){
        var id = $("#hospitalization_select_static").val();

        $.ajax({
            url:"${APP_PATH}/hospitalization/" + id,
            type:"GET",
            success:function(result){
                var hospitalization = result.extend.hospitalization;

                var hospitalizationIdTd = $("<td></td>").append(hospitalization.id);
                var hospitalizationNameTd = $("<td></td>").append(hospitalization.name);
                var hospitalizationCostTd = $("<td></td>").append(hospitalization.cost);
                var deleteBtn = $("<button></button>").addClass("btn btn-danger btn-sm delete_hospitalization_btn ")
                    .append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("移除");
                //为编辑按钮添加一个自定义的属性，来表示当前预约id
                deleteBtn.attr("hospitalization-id", hospitalization.id);
                var btnTd = $("<td></td>").append(deleteBtn);

                $("<tr></tr>").append(hospitalizationIdTd)
                    .append(hospitalizationNameTd)
                    .append(hospitalizationCostTd)
                    .append(btnTd)
                    .appendTo("#hospitalization_table tbody");

                hospitalizationArray = hospitalizationArray + hospitalization.id + ",";
    ...
    //点击移除医疗项目
    $(document).on("click", ".delete_hospitalization_btn", function () {
        cost -= $(this).parents('tr:first')[0].children[2].innerText;
        $(this).parents('tr:first').remove();

        if(hospitalizationArray.search($(this).attr("delete-id")) != -1 ){                                              //true
            hospitalizationArray = hospitalizationArray.replace("1,", "");
        }
    });

    //确认医疗项目
    $("#hospitalization_pay_btn").click(function(){

        $("#bill_set_input").val(cost);
    });

    //添加药品信息
    $(document).on("click", "#medicine_set_input", function () {
        //1.医疗信息点击 需弹出 模态框
        getMedicine("#medicineAddModal select");
    ...
    //开药
    $("#medicine_add_btn").click(function(){
        var id = $("#medicine_select_static").val();

        $.ajax({
            url:"${APP_PATH}/medicine/" + id,
            type:"GET",
            success:function(result){
                var medicine = result.extend.medicine;

                var medicineIdTd = $("<td></td>").append(medicine.id);
                var medicineNameTd = $("<td></td>").append(medicine.name);
                var medicineCostTd = $("<td></td>").append(medicine.cost);
                var deleteBtn = $("<button></button>").addClass("btn btn-danger btn-sm delete_medicine_btn ")
                    .append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("移除");
                //为编辑按钮添加一个自定义的属性，来表示当前预约id
                deleteBtn.attr("medicine-id", medicine.id);
                var btnTd = $("<td></td>").append(deleteBtn);

                $("<tr></tr>").append(medicineIdTd)
                    .append(medicineNameTd)
                    .append(medicineCostTd)
                    .append(btnTd)
                    .appendTo("#medicine_table tbody");

                medicineArray = medicineArray + medicine.id +",";
    ...
    //点击移除药品
    $(document).on("click", ".delete_medicine_btn", function () {
        cost -= $(this).parents('tr:first')[0].children[2].innerText;
        $(this).parents('tr:first').remove();

        if(medicineArray.search($(this).attr("delete-id")) != -1 ){                                              //true
            medicineArray = medicineArray.replace("1,", "");
        }
    });

    //确认医疗项目
    $("#medicine_pay_btn").click(function(){

        $("#bill_set_input").val(cost);
    });

    //确认就医信息
    $("#see_save_btn").click(function(){

        json = {
                meetId:$("#id_set_input").val(),
                customerName:$("#customer_set_input").val(),
                remark:$("#remark_set_input").val(),
                hospitalizationArray:hospitalizationArray,
                medicineArray:medicineArray,
                cost:cost
        };

        $.ajax({
            url:"${APP_PATH}/meetsDoctor/saveMeet",
            type:"POST",
            data: json,
            success:function(result){
                if(result.code = 100){
