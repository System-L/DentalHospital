...
<!-- 就诊记录查看的模态框 -->
<!-- Modal -->
<div class="modal fade" id="recordsGetModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">就诊记录查看</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <button class="btn btn-primary col-sm-2 bill_btn">查看详情</button>
                        <label class="col-sm-2 control-label">时间:</label>
                        <div class="col-sm-4">
                            <input type="text" readonly="readonly" name="date" class="form-control" id="date_get_input" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">备注:</label>
                        <div class="col-sm-10">
                            <textarea name="remark" readonly="readonly" class="form-control" id="remark_get_input" style="height:420px;"></textarea>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <nav id="sidebar" role="navigation" data-step="2"
            ...
            <div class="sidebar-collapse menu-scroll">
                <ul id="side-menu" class="nav">
                    ...
                    <li class="active"><a href="records.jsp"><i class="glyphicon glyphicon-list-alt">
        ...
        <div id="page-wrapper">
            ...
            <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
                <div class="page-header pull-left">
                    <div class="page-title">我的就诊记录</div>
                </div>
                    ...
                    <div class="col-md-4 col-md-offset-4">
                        <div class="form-group">
                            <div class="col-sm-6 col-sm-offset-5">
                                <input type="date" name="name" class="form-control" id="begin_time_input" />
                            </div>
                            <div class="col-sm-1">
                                <p style="height:34px;line-height: 34px;">到</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <div class="col-sm-6">
                                <input type="date" name="name" class="form-control" id="end_time_input" />
                            </div>
                            <div class="col-sm-1">
                                <button class="btn btn-primary" id="records_search_modal_btn">查询</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function to_page(pn) {
        ...
        if(end_time != ""){
            var Url = "${APP_PATH }/records/search";
            var data = "pn=" + pn + "&beginTime=" + begin_time + "&endTime=" + end_time;
        }else{
            var Url = "${APP_PATH }/records";
            var data = "pn=" + pn;
        }
        $.ajax({
            url: Url,
    ...
    function build_records_table(result){
        ...
        $.each(records,function(index,item){
            var recordsIdTd = $("<td></td>").append(item.id);
    ...
    $(document).on("click",".more_btn",function(){
        //查出显示就诊信息
        getRecords($(this).attr("more-id"));
        $("#recordsGetModal").modal({
            backdrop:"static"
        });
    });
    function getRecords(id){
        $.ajax({
            ...
            type:"GET",
            success:function(result){
                var recordData = result.extend.record;
                $("#customer_get_input").val(recordData.customerName);
                $("#doctor_get_input").val(recordData.doctor.name);
                $("#cost_get_input").val(recordData.cost);
                $("#remark_get_input").val(recordData.remark);
                $("#date_get_input").val(new Date(parseInt(recordData.date)).toLocaleDateString().replace(/:\d{1,2}$/,' '));
            }
        })
    }

    //根据日期查询就诊记录
    $("#records_search_modal_btn").click(function(){
        to_page(1);
    })
